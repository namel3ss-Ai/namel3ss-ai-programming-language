...
The migrator rewrites legacy headers (`flow "name":`, `step "name":`, `page "name":`, etc.) to `... is "name":` and replaces `let x = ...` / `set state.x = ...` with `let x be ...` / `set state.x be ...`. Arithmetic operators remain unchanged.
